...
function licenseMapText(license) {
  const names = Array.isArray(license?.allowedMapNames) ? license.allowedMapNames.filter(Boolean) : [];
  const slugs = Array.isArray(license?.allowedMaps) ? license.allowedMaps.filter(Boolean) : [];
  const values = names.length ? names : slugs;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setToolResult(
        ...
        "success",
        "สร้างสคริปต์แล้ว",
        `${data.license?.hwidBound ? "คีย์นี้ผูกเครื่องแล้ว ถ้าย้ายเครื่องให้ใช้ Reset Device ก่อน" : "คัดลอกสคริปต์ไปรันได้เลย ระบบจะผูก HWID ตอนรันครั้งแรก"} • แมพ: ${licenseMapText(data.license)}`,
